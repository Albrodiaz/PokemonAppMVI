...
val Type.pokemonResource: PokemonRes get() = when(this) {
    Type.BUG -> PokemonRes(icon = R.drawable.bug, background = R.drawable.bugbg)
    Type.DARK -> PokemonRes(icon = R.drawable.dark)
    ...
    Type.FLYING -> PokemonRes(icon = R.drawable.flying)
    ...
    Type.ICE -> PokemonRes(icon = R.drawable.ice)
    ...
    Type.STEEL -> PokemonRes(icon = R.drawable.steel)
